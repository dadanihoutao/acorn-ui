...
describe('Alert.vue', () => {
  ...
  it('should support centering text', () => {
    ...
    //class
    const rootNode = wrapper.find('.a-alert')
  ...
  it('should not render close icon when closable is false', () => {
    ...
    expect(wrapper.find('.a-alert__close').exists()).toBe(false)
  })
  ...
  it('should toggle visibility when open and close methods are called', async () => {
    ...
    expect(wrapper.find('.a-alert').attributes().style).toBe('display: none;')
    await wrapper.vm.open()
    expect(wrapper.find('.a-alert').attributes().style).toBe('')
  })
})
